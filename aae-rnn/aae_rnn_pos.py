...
disc_scheduler = torch.optim.lr_scheduler.StepLR(disc_optimizer, step_size=50, gamma=0.33) # reduce the learning every 20 epochs by a factor of 10
ae_scheduler = torch.optim.lr_scheduler.StepLR(ae_optimizer, step_size=50, gamma=0.33) # reduce the learning every 20 epochs by a factor of 10
...
def ae_loss(y, yhat, vis, disc_fake_output):
    # function parameters
    # y: encoder input
    ...
    return _total_loss, _pos_loss, _fake_loss
...
def ae_train_step(target_poses, joint_vis):
    ...
    encoder_output = encoder(target_poses_norm)
    pred_poses_norm = decoder(encoder_output)
    
    # let discriminator output its fake assessment of the encoder ouput
    with torch.no_grad():
        disc_fake_output = disc_prior(encoder_output)
    
    _ae_loss, _ae_pos_loss, _ae_prior_loss = ae_loss(target_poses_norm, pred_poses_norm, joint_vis, disc_fake_output) 
    ...
    return _ae_loss, _ae_pos_loss, _ae_prior_loss
...
def train(train_dataloader, test_dataloader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for train_batch_pose, train_batch_vis in train_dataloader:
            ...
            _disc_prior_train_loss = disc_prior_train_step(train_batch_pose)
            ...
            _ae_loss, _ae_pos_loss, _ae_prior_loss = ae_train_step(train_batch_pose, train_batch_vis)
        ...
        print ('epoch {} : ae train: {:01.4f} ae test: {:01.4f} disc prior {:01.4f} pos {:01.4f} prior {:01.4f} time {:01.2f}'.format(epoch + 1, ae_train_loss_per_epoch, ae_test_loss_per_epoch, disc_prior_loss_per_epoch, ae_pos_loss_per_epoch, ae_prior_loss_per_epoch, time.time()-start))
...
def encode_sequences(pose_sequence, frame_indices):
    ...
    for excerpt_index in range(seq_excerpt_count):
        ...
        with torch.no_grad():
            latent_vector = encoder(excerpt_norm)
