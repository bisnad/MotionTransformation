...
class Mocap_Tools:
    ...
    def mocap_to_npz(self, all_mocap_data):
        """
        Converts a list of mocap_data dictionaries into an NPZ-compatible dictionary map.
        It expects quaternions in the `rot_local` fields. Global coordinates are calculated dynamically if missing.
        """
        npz_dict = {}
        
        for skel_idx, mocap_data in enumerate(all_mocap_data):
            fps = mocap_data.get("frame_rate", 30.0)
            motion = mocap_data["motion"]
            
            pos_local = motion["pos_local"]
            rot_local = motion["rot_local"]
            
            # Guarantee world coordinates exist
            if "pos_world" not in motion or "rot_world" not in motion:
                pos_world, rot_world = self.local_to_world(rot_local, pos_local, mocap_data["skeleton"])
            else:
                pos_world = motion["pos_world"]
                rot_world = motion["rot_world"]
                
            frame_count = pos_local.shape[0]
            timestamps = np.arange(frame_count) / fps
            
            # Reshape from (Frames, Joints, Axes) -> (Frames, Joints * Axes)
            pos_local_flat = pos_local.reshape(frame_count, -1)
            pos_world_flat = pos_world.reshape(frame_count, -1)
            rot_local_flat = rot_local.reshape(frame_count, -1)
            rot_world_flat = rot_world.reshape(frame_count, -1)
            
            npz_dict[f'/mocap/{skel_idx}/joint/pos_local_values'] = pos_local_flat
            npz_dict[f'/mocap/{skel_idx}/joint/pos_local_timestamps'] = timestamps
            npz_dict[f'/mocap/{skel_idx}/joint/pos_world_values'] = pos_world_flat
            npz_dict[f'/mocap/{skel_idx}/joint/pos_world_timestamps'] = timestamps
            
            npz_dict[f'/mocap/{skel_idx}/joint/rot_local_values'] = rot_local_flat
            npz_dict[f'/mocap/{skel_idx}/joint/rot_local_timestamps'] = timestamps
            npz_dict[f'/mocap/{skel_idx}/joint/rot_world_values'] = rot_world_flat
            npz_dict[f'/mocap/{skel_idx}/joint/rot_world_timestamps'] = timestamps
