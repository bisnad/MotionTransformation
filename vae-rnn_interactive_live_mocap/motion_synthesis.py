...
class MotionSynthesis():
    ...
    def __init__(self, config):
        ...
        # new : live mocap sequence
        self.live_seq = torch.tensor([1.0, 0.0, 0.0, 0.0]).repeat(self.seq_window_length, self.joint_count)
        self.live_pose = None
        self.live_seq_changed = False
    ...
    def setLiveSeq(self, rotLocal):
        ...
        rotLocal = torch.tensor(rotLocal).to(dtype=torch.float32)
        rotLocal = rotLocal.reshape(self.joint_count, 4)
        
        self.live_pose = rotLocal
        self.live_seq_changed = True
    ...
    def changeLiveSeq(self):
        
        self.live_seq = torch.cat([self.live_seq[1:, ...], self.live_pose.reshape(1, self.pose_dim)], axis=0)
        self.live_seq_changed = False
    ...
    def _gen(self):
        ...
        # encode live seq window 1 and orig seq window 2
        
        live_seq_window = self.live_seq.reshape(1, self.seq_window_length, self.pose_dim).to(self.device)
        ...
        with torch.no_grad():
            encoder_output_live = self.model_encoder(live_seq_window)
            encoder_output_live_mu = encoder_output_live[0]
            encoder_output_live_std = encoder_output_live[1]
            mu_live = torch.tanh(encoder_output_live_mu)
            std_live = torch.abs(torch.tanh(encoder_output_live_std)) + 0.00001
            encoding_live = self._reparameterize(mu_live, std_live)
            
            encoder_output_orig = self.model_encoder(orig_seq_window)
            encoder_output_orig_mu = encoder_output_orig[0]
            encoder_output_orig_std = encoder_output_orig[1]
            mu_orig = torch.tanh(encoder_output_orig_mu)
            std_orig = torch.abs(torch.tanh(encoder_output_orig_std)) + 0.00001
            encoding_orig = self._reparameterize(mu_orig, std_orig)

        # mix live encoding and orig encoding
        encoding = encoding_live * (1.0 - self.encoding_mix) + encoding_orig * self.encoding_mix
