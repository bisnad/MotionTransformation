...
def train(train_dataloader, test_dataloader, epochs):
    ...
    for epoch in range(epochs):
        ...
        # Step the scheduler and decay the base learning rate at cycle boundaries
        if epoch > 0:
            if epoch % int(cycle_length) == 0:
...
def decode_sequence_encodings(sequence_encodings, seq_overlap, base_pose):
    ...
    for excerpt_index in range(seq_excerpt_count):
        ...
        # Orthogonalize sequence immediately
    ...
    # Normalize accumulated rotation matrices via Partition of Unity
    weight_accum[weight_accum == 0.0] = 1.0
    avg_matrices = gen_sequence_accum / np.expand_dims(weight_accum, axis=-1)

    # Extract clean 6D vectors from blended matrices
    ...
    if train_root_trajectory:
        ...
        # Un-normalize to real-world scale
